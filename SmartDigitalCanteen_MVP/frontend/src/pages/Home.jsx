...
export default function Home() {
  ...
  const [customerName, setCustomerName] = useState(localStorage.getItem("CUSTOMER_NAME") || "Guest");
  ...
  const addToCart = (item) => {
    setCart(prev => {
      ...
      if (existing) return prev.map(p => p._id === item._id ? { ...p, qty: p.qty + 1 } : p);
    ...
    <div className="container mx-auto px-4 py-6 grid grid-cols-1 md:grid-cols-3 gap-8">
      ...
      <div className="md:col-span-2 flex flex-col gap-6">
        ...
        <div className="bg-white rounded-2xl shadow-lg p-5">
          <label className="text-sm text-gray-600 block mb-2 font-medium">Your name (for order tracking)</label>
          <input 
            value={customerName} 
